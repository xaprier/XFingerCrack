...
$updatedContent = $innoSetupContent `
    ...
    -replace '\$CHANGEMEWITHPATH\\resources', "$scriptDir\resources" `
    -replace '\$CHANGEMEWITHPATH\\build\\install\\Output', "$scriptDir\build\install\Output" `
    -replace '\$CHANGEMEWITHPATH\\build\\install\\\{#MyAppExeName\}', "$scriptDir\build\install\bin\{#MyAppExeName}" `
    -replace '\$CHANGEMEWITHPATH\\build\\install\\\*', "$scriptDir\build\install\bin\*" `
